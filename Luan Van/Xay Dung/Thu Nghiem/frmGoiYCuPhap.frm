...
Begin VB.Form frmGoiYCuPhap 
   ...
   End
End
Attribute VB_Name = "frmGoiYCuPhap"
...
Private Function Tachtu(ByVal st As String) As String()
Dim tu() As String
...
Public Function SimilarWord() As Integer
On Error GoTo Result
Dim tu() As String
Dim goiy1() As String, goiy2() As String, goiy3() As String
Dim i As Long, j As Long, dem As Long, k As Long
Dim st As String, nho As Long
Dim loca As New clsLocation
    SimilarWord = 0
    tu = Tachtu(UniLCase(txtAmTiet.Text))
    nho = UBound(tu)
    For i = 1 To nho
        dem = 0
        For j = 1 To Dic.PCount
            st = Dic.WordPage(j)
            If SimilarSyllable(tu(i), Len(tu(i)), st, Len(st)) Then
                dem = dem + 1
                Select Case i
                    Case 1: ReDim Preserve goiy1(dem)
                            goiy1(dem) = st
                    Case 2: ReDim Preserve goiy2(dem)
                            goiy2(dem) = st
                    Case 3: ReDim Preserve goiy3(dem)
                            goiy3(dem) = st
                End Select
            End If
        Next j
    ...
    Select Case nho
        Case 1:
            For i = 1 To UBound(goiy1)
                lstTuongDuong.AddItem goiy1(i)
            Next i
        Case 2:
            For i = 1 To UBound(goiy1)
                For j = 1 To UBound(goiy2)
                    st = goiy1(i) & " " & goiy2(j)
                    Set loca = Dic.FindWord(st)
        ...
        Case 3:
            For i = 1 To UBound(goiy1)
                For j = 1 To UBound(goiy2)
                    For k = 1 To UBound(goiy3)
                        st = goiy1(i) & " " & goiy2(j) & " " & goiy3(k)
                        Set loca = Dic.FindWord(st)
                        If Not (loca Is Nothing) And loca.ok = 0 Then
                            lstTuongDuong.AddItem st
                        End If
